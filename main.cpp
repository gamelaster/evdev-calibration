...
int main(int argc, char *argv[])
{
	...
	while (isRunning) {
		...
		if (corner == 4) {
			...
			printf("Do you want to apply configuration? [Y/n]: ");
			char applyCalibration;
			scanf(" %c", &applyCalibration);
			if (applyCalibration == 'Y' || applyCalibration == 'y') {
				system(xinputCommand);
			}
			char xorgFile[512];
			snprintf(xorgFile, sizeof(xorgFile), "Section \"InputClass\"\n"
			"\tIdentifier      \"calibration\"\n"
			"\tMatchProduct    \"%s\"\n"
			"\tOption \"TransformationMatrix\" \"%.6f 0 %.6f 0 %.6f %.6f 0 0 1\"\n"
			"EndSection", deviceName, c0, c1, c2, c3);
			printf("Please, write this configuration to: /usr/share/X11/xorg.conf.d/99-calibration.conf\n%s\n", xorgFile);
			isRunning = false;
		}
		SDL_Delay(1000 / 64);
	}
